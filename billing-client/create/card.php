...
if ((!isset($_POST['stripeToken'])) || ($_POST['stripeToken'] == '')) { header('Location: ../add/card.php?error=1'); exit();}
...
try {
    $customer = \Stripe\Customer::retrieve($currentcustomer);
    $newcard = $customer->sources->create(["source" => $_POST['stripeToken']])->__toArray(true);;
}
catch (\Stripe\Error\Base $e) { $err = $e->getJsonBody()['error']['message']; }
if(isset($err) || $err != '') {
    header("Location: ../index.php?a1=" . $err);
}
else {
    if (isset($_POST['defaultcard']) && !empty($_POST['defaultcard'])) {
        try {
            $customer = \Stripe\Customer::update($currentcustomer, ['default_source' => $newcard['id']]);
        }
        catch (\Stripe\Error\Base $e) { $err = $e->getJsonBody()['error']['message']; }
        if(isset($err) || $err != '') {
            header("Location: ../index.php?a1=" . $err);
