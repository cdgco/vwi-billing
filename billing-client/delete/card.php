...
if ((!isset($_POST['verified'])) || ($_POST['verified'] == '')) { $r1 = 'Verification Missing'; }
elseif ((!isset($_POST['card'])) || ($_POST['card'] == '')) { $r1 = 'Card ID Missing'; }
elseif ((!isset($_POST['customer'])) || ($_POST['customer'] == '')) { $r1 = 'Customer ID Missing'; }
try {
    $customer = \Stripe\Customer::retrieve($_POST['customer']);
    $customer->sources->retrieve($_POST['card'])->delete();
}
catch (\Stripe\Error\Base $e) { $err = $e->getJsonBody()['error']['message']; }
if(isset($err) || $err != '') { $r1 = $err; } 
else {
    $r1 = '0';
}
